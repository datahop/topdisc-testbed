...
func runMultiTopicWorkload(all []nodeRec, numTopics int, zipfS float64, seed int64, registerWait, searchTimeout, regProbePeriod, registerStagger, registerWindow time.Duration, metricsOut string, pacing searchPacing) {
	if seed == 0 {
	...
	if registerWindow > 0 {
		if d := time.Until(regStart.Add(registerWindow)); d > 0 {
			time.Sleep(d)
		}
	}
