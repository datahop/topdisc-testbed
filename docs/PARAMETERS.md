...
| Measured popularity | Nodes over chains as the discv5 network has them (crawl of 2026-09-18: 29k live nodes, 5 chains with ≥1000, a long tail). | `population.topic_model: models/crawl-2026-09-18.json`, `topics: K` keeps the K largest chains, the K-th takes the tail (0 = all 24) | ✓ | ✓ | `testbed preview <scenario>` prints the sizes and churn it yields; `cmd/crawl/model.py` fits a model from any crawl |
...
| Bandwidth | Plan: 20 KB/s per node. | simnet: `network.bandwidth_mibps` (per link); local/Grid'5000: `testbed.wan.rate_kbps` (default 160 = the plan's figure; the g5k scenarios set 1000 so the cap does not hide load as queueing) | ✓ | ◐ | AWS: not capped |
...
| Measured churn per topic | Each node leaves, stays away and comes back (or not) as its chain's nodes did in the 2026-09-18 crawl: arrivals, permanent leaves and returns per hour differ 10x between consensus and execution chains. | `session_churn.model: models/crawl-2026-09-18.json` with the same file as `population.topic_model`; `window_real_hours` (0 = real time) compresses a day into the search window | ✓ | ✓ | Fit from 2-min probes and a 30-min gap; returns come out ~30 % low against the crawl, arrivals and permanent leaves within 20 % (`testbed preview`) |
