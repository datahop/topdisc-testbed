...
type Topic struct {
	...
	SearchYieldFloor    int   `json:"search_yield_floor"`
	SearchAuxRadius     int   `json:"search_aux_radius"`
...
func Generate(cfg scenario.Config, hosts func(idx int) Host, t0 int64, modelDir string) ([]Assignment, error) {
	...
	for i := range out {
		...
		a.Topic = Topic{AdLifetimeMs: sc.Topic.AdLifetime.Milliseconds(), AdCacheSize: sc.Topic.AdCacheSize,
			...
			TopicNodesLimit: sc.Topic.TopicNodesLimit, AuxNodesLimit: sc.Topic.AuxNodesLimit, SearchYieldFloor: sc.Topic.SearchYieldFloor, SearchAuxRadius: sc.Topic.SearchAuxRadius,
